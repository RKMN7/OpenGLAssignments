...
int main()
{
	...
	Shader Head("vs.glsl", "fsHead.glsl");
	Shader Body("vs.glsl", "fsBody.glsl");
	Shader Limb("vs.glsl", "fsLimb.glsl");
	Shader LimbAnim("vsAnim.glsl", "fsLimb.glsl");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		Head.use();
		Head.setMat4("proj", cam);
		glBindVertexArray(VAO[4]);
		glDrawArrays(GL_TRIANGLE_FAN, 0, 12);


		Body.use();
		Body.setMat4("proj", cam);

		glBindVertexArray(VAO[0]);
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);

		Limb.use();
		Limb.setMat4("proj", cam);
